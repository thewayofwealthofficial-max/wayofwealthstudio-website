...
export function checkDraft({ subject, preview, body_plain }, { type, allowedLinks = [], sourceText = '', shapeText = '', shapeSubject = '', blockedNames = new Set(), passagesText = '', requireJoelWords = false } = {}) {
  ...
  if (/\bI hear (?:this |it |that )?(?:from [^.\n]{0,40})?(?:all the time|a lot|so often|most|constantly)\b|\b(?:a lot of|lots of|many|so many) (?:people|clients|practitioners|of you) (?:tell|say|ask|have told)\b|\bthe (?:objection|question) I (?:hear|get) most\b/i.test(all)) problems.push('Claims how often Joel hears something ("I hear this all the time", "a lot of people tell me"). Unverifiable, cut it.');
